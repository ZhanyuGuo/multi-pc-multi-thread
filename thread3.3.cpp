/* -------------------------------------------------------------
 * 
 * File_name: thread3.3.cpp
 * 
 * Note: Change sin() to log10()
 *       Change double(i%100) to double(i + 1)
 *  
 * -----------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
...
double NewSum(const double data[], const int len)
{
    ...
    for (int i = 0; i < len; i++) rlt += log10(sqrt(data[i]));
...
void* threadFunc(void *arg)
{
    ...
    // {
    //     doubleResults[who] += log10(sqrt(rawDoubleData[i]));
    // }
